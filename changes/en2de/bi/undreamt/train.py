...
def main_train():
    ...
    if args.is_load_model:
		...
		src_decoder._share(trg_decoder.get_attention(), trg_decoder.get_net(), update_attnF = False)
        trg_decoder._share(src_decoder.get_attention(), src_decoder.get_net(), update_attnF = False)
    ...
    if args.validation_output is not None:
        src2src_output = '{0}.{1}.src2src'.format(now, args.validation_output)
        trg2trg_output = '{0}.{1}.trg2trg'.format(now, args.validation_output)
        src2trg_output = '{0}.{1}.src2trg'.format(now, args.validation_output)
        trg2src_output = '{0}.{1}.trg2src'.format(now, args.validation_output)
    ...
    def save_models(name):
        print("saving_model:{0} iters".format(name))
        try:
            os.mkdir('./models/'+str(name))
        except Exception as e:
            pass
        torch.save(src2src_translator, os.path.join('models', str(name), '{0}.src2src.pth'.format(args.save)))
        torch.save(trg2trg_translator, os.path.join('models', str(name), '{0}.trg2trg.pth'.format(args.save)))
        torch.save(src2trg_translator, os.path.join('models', str(name), '{0}.src2trg.pth'.format(args.save)))
        torch.save(trg2src_translator, os.path.join('models', str(name), '{0}.trg2src.pth'.format(args.save)))
    def save_trainers():
        print("saving trainers:step--"+str(step))
        torch.save({
            'src2src_optimizers':src2src_optimizers,
            'trg2trg_optimizers':trg2trg_optimizers,
            'src2trg_optimizers':src2trg_optimizers,
            'trg2src_optimizers':trg2src_optimizers,
            'step':step,
        }, os.path.join('trainers', 'trainers.pth'))



    # Training

    if args.is_load_optimizer:
        step_t = step+1
    else:
        step_t = 1
    for step in range(step_t, args.iterations + 1):
        for trainer in trainers:
            trainer.step()
        # if not step % 100  :
            # print("{0}--ITER:{1}".format(datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S'),step))
        if args.save is not None and args.save_interval > 0 and step % args.save_interval == 0:
            save_models('it{0}'.format(step))
            torch.cuda.empty_cache()
        # if args.save is not None and args.save_train_interval > 0 and step % args.save_train_interval == 0:
        #     save_trainers()
        if step % args.log_interval == 0:
            print()
            print('STEP {0} x {1} time:{2}'.format(step, args.batch, datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')))
    ...
    def log(self, step=0):
        ...
        for id, validator in enumerate(self.validators):
            ...
            if self.output_prefix is not None:
                f = open(os.path.join('log', '{0}.{1}.step={2}.txt'.format(self.output_prefix, id, step)), mode='w',
